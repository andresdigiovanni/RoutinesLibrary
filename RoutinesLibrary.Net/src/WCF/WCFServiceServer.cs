...
namespace RoutinesLibrary.Net.WCF
{
    public class WCFServiceServer
    {
            ...
            int servicePort = 8000)
        {
            ...
            Uri hostURI = new Uri("http://" + Environment.MachineName + ":" + System.Convert.ToString(servicePort) +
                                  "/" + serviceName);
            ...
            try
            {
                ...
                if (serviceHost.Description.Behaviors.Contains(typeof(ServiceDebugBehavior)))
                {
                    ((ServiceDebugBehavior) (serviceHost.Description.Behaviors[typeof(ServiceDebugBehavior)]))
                ...
                if (serviceHost.Description.Behaviors.Contains(typeof(ServiceMetadataBehavior)))
                {
                    ((ServiceMetadataBehavior) (serviceHost.Description.Behaviors[typeof(ServiceMetadataBehavior)]))
                        .HttpGetEnabled = true;
                }
                else
                {
